...
parseInp s = mapMaybe parseInp' (wordsWhen (== '\n') s)

parseInp' :: String -> Maybe (PasswordRule, String)
parseInp' line = case wordsWhen (\c -> c == ' ' || c == ':') line of
  [amts, char, testcase] ->
    ( case wordsWhen (== '-') amts of
        [numa, numb] ->
          (readMaybe numa :: Maybe Int) >>= \a ->
            (readMaybe numb :: Maybe Int) >>= \b ->
              Just
                ( PasswordRule {ruleA = a, ruleB = b, ruleChar = head char},
                  testcase
                )
        _ -> Nothing
    )
...
countValid ::
  [(PasswordRule, String)] -> (PasswordRule -> String -> Bool) -> Int
