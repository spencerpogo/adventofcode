...
countTrees'    m         right  down   x      y      trees  width = 
    if y < (length m) then
        let newTress = if ((m !! y) !! (x `mod` width)) == Tree
            then trees + 1
            else trees
...
part2 :: Course -> String
part2 m = show $ (countTrees m 1 1) * (countTrees m 3 1) * (countTrees m 5 1) * (countTrees m 7 1) * (countTrees m 1 2)
