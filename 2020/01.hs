...
find2020num' (x : xs) possible = if Set.member x possible
  then Just (x * (2020 - x))
  else if x < 2020
    then find2020num' xs (Set.insert (2020 - x) possible)
    else find2020num' xs possible
...
find2020triplet' possibleA possibleBMap possibleB (x : xs) = do
  if Set.member x possibleB
    then Map.lookup x possibleBMap >>= \a1 ->
      let a = 2020 - a1
          b = x
          c = 2020 - (a + b)
      in  Just (a, b, c)
