...
find2020num' (x : xs) possible
  | Set.member x possible = Just (x * (2020 - x))
  | x < 2020 = find2020num' xs (Set.insert (2020 - x) possible)
  | otherwise = find2020num' xs possible

find2020num :: [Integer] -> Maybe Integer
find2020num inp = find2020num' inp Set.empty

part1 :: String -> String
part1 s = maybe "Error" show (find2020num (parseInp s))
...
findB ::
  ...
  Set.Set Integer ->
  (Map.Map Integer Integer, Set.Set Integer)
...
findB x (y : ys) possibleBMap possibleB = do
  if x < y
    ...
    else findB x ys possibleBMap possibleB
      ...
      if x < 2020
        then
          let newPossibleA = Set.insert (2020 - x) possibleA
              (newPossibleBMap, newPossibleB) =
                findB x (Set.elems newPossibleA) possibleBMap possibleB
           in find2020triplet' newPossibleA newPossibleBMap newPossibleB xs
        else find2020triplet' possibleA possibleBMap possibleB xs
